...
Review loaded context before writing any code. If the connector is unavailable, read `docs/strategy/PRODUCT_DIRECTION.md` and inspect the repository directly; never claim that memory was queried when it was not.

## Product Boundary

Cortex preserves verified engineering state so a human or coding agent can resume work safely across sessions, tools, and vendors. Prefer task, evidence, decision, artifact, verification, and handoff concepts over generic memories. Read `docs/strategy/README.md` before architecture work.
